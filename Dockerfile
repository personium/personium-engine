...
ENV JAVA_OPTS="\
        -Dio.personium.configurationFile=/personium/personium-engine/conf/personium-unit-config.properties"

WORKDIR /usr/local/tomcat/webapps/
COPY --from=build /usr/src/app/target/personium-engine.war .
